...
from coinbase.credentials_file import Credentials, CredentialsFile, DEFAULT_PATH


def test_default_path_is_under_the_home_directory():
    assert DEFAULT_PATH == Path.home() / ".coinbase" / "credentials.yaml"


def test_credentials_file_loads_api_key_and_secret(tmp_path):
    path = tmp_path / "credentials.yaml"
    path.write_text(yaml.dump({"api_key": "organizations/x/apiKeys/y", "api_secret": "-----BEGIN EC PRIVATE KEY-----\n"}))
...
def test_credentials_file_raises_when_a_key_is_missing(tmp_path):
    path = tmp_path / "credentials.yaml"
    path.write_text(yaml.dump({"api_key": "organizations/x/apiKeys/y"}))
