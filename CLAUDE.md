...
- **A strategy is a design plus a policy.** `SignalModel` in `coinbase/ga/strategy_evaluator.py` is the design — the function turning a candle into a `signal_score`. `GaStrategy` is the policy that acts on it: the three bands, thresholds and sizing. `SignalDesign` is the one place mapping a design *name* to its model and to the `WeightScaling` the GA rescales its genomes with (L1 for `linear`; a network's internal parameters must not be rescaled at all). `linear` is the only design today and the default everywhere. Every `strategy.json` records its design, and loading one whose design this build lacks raises — a genome is otherwise just a bag of numbers that any design would score plausibly and wrongly. Adding a design is additive: two can be papered side by side as separate `algos:` entries in `paper.yaml`.
...
- `Ledger` charges **fees per leg and hourly borrow interest on shorts**, both off by default (`strategy.fee_bps` / `strategy.borrow_bps_per_hour`, `0.0`), so an existing config reproduces its old numbers exactly. `Trade.profit()` stays gross — every `strategy.json` and `index.csv` row predating costs is denominated in it — and `net_profit()` is what the GA selects on. Interest is still billed in one lump at close rather than accruing continuously, so it does **not** decay the margin level: a short that Binance would liquidate on interest alone survives here, and `equity()`/`max_drawdown` read slightly rich while a short is open.
