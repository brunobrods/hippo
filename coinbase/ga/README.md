...
| `strategy_evaluator.py` | Holds the two halves of a strategy: the **design** (`SignalModel` — the function turning a candle into a `signal_score`; `linear` is the only one today) and the **policy** (`GaStrategy` — the three bands, thresholds and sizing that act on it). Walks the candles as a single-position backtest and reports profit **net of fees and borrow interest** — this *is* the `FitnessFunction` the GA evolves against |
...
| | `design` | Which model design turns a candle into a `signal_score`. `"linear"` (the default, and the only one this build has) is the L1-normalized weighted sum. Recorded in every `strategy.json`, so a genome declares what produced it and loading one under a design this build lacks raises rather than mis-scoring. Group runs by it with `--group-by design` |
