...
class Backtest:
    ...
    def run(self) -> BacktestResult:
        ...
        for row in records:
            ...
            #
            # Liquidation is checked FIRST and the take-profit second, and the
            # order is the whole safety of this: a candle that reaches both
            # gives no way to know which came first, so the adverse one is
            # assumed to have. Reversing these two lines would invent money.
            ledger.liquidate(high, low)
            self._take_profit(ledger, high, low)
    ...
    def _take_profit(self, ledger: Ledger, high: float, low: float) -> None:
        ...
        if exit_order.reached_by(high, low):
            ledger.force_close(exit_order.price())
